...
public class Login extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 180, 170, 50));
        getContentPane().add(t1, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 130, 180, 30));
        ...
        getContentPane().add(jPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 190, 160, 40));

        jLabel1.setFont(new java.awt.Font("宋体", 1, 36)); // NOI18N
        jLabel1.setText("User ID");
        jPanel2.add(jLabel1);

        getContentPane().add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 120, 160, 50));
        getContentPane().add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(70, 90, -1, -1));
        getContentPane().add(p, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 200, 180, 30));

        jLabel6.setIcon(new javax.swing.ImageIcon(getClass().getResource("/photo/photo1.jpg"))); // NOI18N
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            ...
            String sql = "select * from user where user_ID=? and user_Password=?";  //检查用户的名字和密码

            pst = con.prepareStatement(sql);
            pst.setString(1, t1.getText());    //获得输入的用户id
            pst.setString(2, p.getText());   //获取用户输入的密码
            rs = pst.executeQuery();
            if (t1.getText().length() == 0) {    //检查用户名是否为空
                ...
                id = t1.getText();
                User s = new User();  //用户名和密码正确则打开用户界面
                s.setVisible(true);
                this.hide();
            } else {
